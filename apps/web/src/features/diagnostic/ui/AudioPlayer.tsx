...
export default function AudioPlayer({id,index,initial,done,review,disabled,onReady,onProgress,onIssue}: {id:string;index:number;initial:number;done:boolean;review:boolean;disabled:boolean;onReady:(i:number,ready:boolean)=>void;onProgress:(i:number,seconds:number,ended:boolean)=>void;onIssue:(message:string)=>void}) {
  ...
  /* Một kênh sự kiện chung cho mọi player: `detail` là index đang phát, -1 nghĩa là dừng hết,
     nhờ vậy thanh thu gọn ở header pause được mà không cần cầm ref của player nào. */
  ...
  const announce=(playing:boolean)=>window.dispatchEvent(new CustomEvent('diagnostic-audio-state',{detail:{index,playing,current:audio.current?.currentTime??0,duration:audio.current?.duration??0}}));
  ...
  return <div className="diag-audio">
    <audio ref={audio} preload="auto" src={`/api/practice/listening/audio/${id}`} onLoadedMetadata={()=>{const el=audio.current!;setDuration(el.duration);if(!initialized.current){el.currentTime=Math.min(position.current,el.duration);initialized.current=true;}}} onCanPlay={()=>{setError(false);onReady(index,true);}} onError={()=>{setError(true);onReady(index,false);onIssue(`Lỗi tải audio ${index+1}`);}} onPlay={()=>{setPlaying(true);announce(true);}} onPause={()=>{setPlaying(false);announce(false);}} onRateChange={()=>{if(!review&&audio.current)audio.current.playbackRate=1;}} onTimeUpdate={()=>{const el=audio.current!;position.current=el.currentTime;setCurrent(el.currentTime);if(!el.paused)announce(true);onProgress(index,el.currentTime,el.ended);}} onEnded={()=>{setPlaying(false);announce(false);onProgress(index,duration,true);}} />
