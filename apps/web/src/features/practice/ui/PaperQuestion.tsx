 ...
 *
 * The import flattened the source document's tables into sentences, so a table * task reads as one line per cell instead of a grid. Restoring the grid needs * the layout stored at import time, which it currently is not. */ /** * Two blank notations reach us from the importer. Most prompts keep the source * document's own marker — "6………………" — and a handful were rewritten with a run
