...
export default function ReadingPassageCard({
  ...
  const rawTitle = passageLabelOf(test)
  ...
  return (
    ...
    >
      <div
        className={`relative h-40 overflow-hidden bg-gradient-to-br ${VISUALS[(index + passage - 1) % VISUALS.length]}`}
      >
        {test.coverImageUrl ? (
          <Image
        ...
        <span
          className={`absolute bottom-0 left-0 rounded-tr-2xl px-5 py-2 text-sm font-bold ${BADGES[passage - 1]}`}
        >
          Passage {passage}
        </span>
      </div>
